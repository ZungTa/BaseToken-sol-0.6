...
contract ERC20 is Context, IERC20 {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        ...
        _totalSupply = _totalSupply.sub(
            amount,
            "ERC20: totalSupply amount exceeds"
        );
...
contract Ownable is Context {
    ...
    event ChangeAdmin(address indexed previousAdmin, address indexed newAdmin);
    ...
    modifier onlyAdminOrOwner() {
        require(
            _admin == _msgSender() || _owner == _msgSender(),
            "Ownable: caller is not admin or owner"
        );
        _;
    }
    ...
    function appointAdmin(address newAdmin) public virtual onlyOwner {
        emit ChangeAdmin(_admin, newAdmin);
        _admin = newAdmin;
    }
}
...
contract Blacklist is Ownable {
    ...
    modifier isBlacklisted() {
        require(blacklist[msg.sender], "Blacklist: msg.sender is blacklisted");
        _;
    }
    ...
    modifier isNotBlacklisted() {
        require(
            ...
            "Blacklist: msg.send is not blacklisted"
        );
        _;
    }
     ...
     * @param addr address
     * @dev return true if the address was added to the blacklist, false if the address was already in the blacklist
     */
     ...
     * @dev return true if at least one address was added to the blacklist,
     * false if all addresses were already in the blacklist
     */
    function addAddressesToBlacklist(address[] memory addrs)
        public
        onlyAdmin
        returns (bool success)
    {
     ...
     * @param addr address
     * @dev return true if the address was removed from the blacklist,
     ...
     * @param addrs addresses
     * @dev return true if at least one address was removed from the blacklist,
...
contract BaseToken is ERC20, Pausable, Blacklist {
    constructor(
        string memory name,
        string memory symbol,
        uint8 decimals,
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public virtual override whenNotPaused isNotBlacklisted returns (bool) {
